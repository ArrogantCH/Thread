## Thread_2 线程传参详解，detach()大坑，成员函数做线程函数
...
int main()
{
	...
	std::thread mytobj(myprint,mvary, mybuf); //如果使用(3)， 这里其实存在一个隐身转换，转换成string，事实上存在，mybuf都被回收了（main函数执行完了）,系统才用mybuf去转string
...
int main()
{
	int mvar = 1;
	int mysecondpar = 12;
	//std::thread mytobj(myprint, mvar, mysecondpar); //我们希望mysecondpar转成A类型对象传递给myprint的第二个参数。这种隐式转换是不安全的
	std::thread mytobj(myprint, mvar, A(mysecondpar)); //这种是安全的
	mytobj.join();
}
```
...
​		只要用临时构造的类对象作为参数传递给线程，那么就一定能够在主线程执行完毕前把现场函数的第二个参数构建出来，从而确保即便detach()子线程了也安全运行

#### 1.2 总结

​	a.若传递int这种简单类型参数，建议都是值传递，不要用引用

​	b.如果传递类对象，避免隐式类型转换，全部都在创建线程这一行就构建出临时对象来，然后在函数参数里用引用来接，否则系统还会构造一次，总共三次对象
...
​	id是个数字，每个线程（不管是主线程还是子线程），每个线程实际上都对应一个数字，而且每个线程对应的这个id也不同，也就是说，不同的线程，它的线程id(数字)必然是不同的
...
{
public:
	mutable int m_i; //mutable 无论怎样都可以修改这个值，包括const （此处是为了测试）
	//类型转换构造函数，可以把一个int转换成类A对象
	A(int a) : m_i(a)
	{
		std::cout << "A(int a) : m_i(a)  构造函数执行" << this << " thread_id = " << std::this_thread::get_id() << std::endl;
...
void myprint2(const A &pmybuf)  //此处必须要const，因为没有使用std::ref
{
...
void myprint2(A &pmybuf) //此处不需要const
{
...
{
    A myobj(10);
	//std::thread mytobj(&A::thread_work, myobj, 15); //不是引用
	std::thread mytobj(&A::thread_work, std::ref(myobj), 15); //引用 
