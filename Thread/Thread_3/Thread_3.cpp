...
std::vector<int> g_v = { 1,2,3 }; //共享数据，只读
...
{
	std::cout << "id为" << std::this_thread::get_id() << "的线程  打印g_v值" << g_v[0] << g_v[1] << g_v[2] << std::endl;
	return;
}
...
int main()
{
	...
	//二 数据共享问题分析
	// (2.1) 只读的数据，是安全稳定的，不需要特别的什么处理手段，直接读就可以
	// (2.2) 有读有写： 2个线程写，8个线程读。 如果代码没有特别处理，程序肯定崩溃
	//       最简单的处理，读的时候不能写，写的时候不能读。2个线程不能同时写，8个线程不能同时读
	// (2,3) 其他案例
	// 数据共享
	// 北京-深圳 火车， 10个售票窗口 卖票     1，2窗口都要订99号座位

	//三 共享数据的保护案例代码
	// 网络游戏服务器。 两个自己创建的线程，一个线程收集玩家命令（用一个数字代表玩家发的命令），并把命令数据写到一个队列中
	// 另外一个线程从队列中取出玩家发送来的命令，解析，然后执行玩家需要的动作
	//vector, list，  list:随机插入和删除数据效率高
