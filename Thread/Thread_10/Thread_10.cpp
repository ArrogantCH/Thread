// Thread_10.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
int main()
{
	...
	// (b)std::launch::async: 强制这个异步任务在新线程上执行，这意味着，系统必须要创建出新线程来运行mythread()
	...
	//  (2)如果用std::async，一般就不会报异常，不会崩溃，因为，如果系统资源紧张导致无法创建新线程的时候，
	//     async这种不加额外参数的调用，就不会创建新线程，而是后续调用了result.get()来请求结果
	//	如果你强制std::async一定要创建新线程，那么就必须使用std::launch::async，承受的带价就是系统资源紧张时，程序崩溃叭
	//  经验：一个程序里，线程是数量不要超过100-200

	//	2.3 std::async不确定性问题的解决
	//  不加额外参数的std::async嗲用，让系统自行决定是否创建新线程
